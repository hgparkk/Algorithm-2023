# 알고리즘 설계 구현 결과: <재귀함수를 이용한 유클리드 알고리즘과 반복문을 이용한 유클리드 알고리즘 결과와 실행시간 비교하기>
...
#### 2. 숫자 쌍을 저장하기 위한 두 배열을 선언한다. 
#### 3. 랜덤한 변수 n개를 배열에 저장한다. (알고리즘을 실행하기 위한 a>=b>=0 조건이 있으므로 b가 더 크면 a와 b를 바꾼다.)
